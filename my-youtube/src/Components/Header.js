...
const Header = () => {
  ...
  // useEffect(() => {
  //   console.log(filterData(filterVideo, searchVideo));
  // }, [filterVideo])
  ...
  return (
    <div className="grid grid-flow-col md:items-center shadow-lg items-center">
      ...
      <div className="flex">
        ...
        {showSuggestions && (
          <div
            className="absolute mt-11 bg-white py-2 px-2 w-[600px] md:w-[400px] sm:w-[150px] xsm:w-[150px] mob:w-[150px] shadow-lg rounded-lg border border-gray-100"
          >
            {suggestions.length > 0 ? (<ul>
              {suggestions.map((s) => (
                <li key={s} className="py-2 px-3 shadow-sm hover:bg-gray-100" onClick={() => {
                  setsearchVideo(s)
                }}>🔍 {s}</li>
              ))}
            </ul>) : (<p>No suggestions found</p>)}
          </div>
        )}
      </div>
